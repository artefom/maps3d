...
public class DistanceFieldInterpolation {
    ...
    public DistanceFieldInterpolation(IsolineContainer container) {
        ...
        this.isolines = new AttributedIsoline[container.size()];
        List<Geometry> occluders = new ArrayList<>(container.size());
        int i = 0;
        for (IIsoline iso : container) {
            isolines[i] = new AttributedIsoline(iso);
     ...
     */
    public void rasterizeline(pixelInfo[][] buf, int x,int y,int x2, int y2, AttributedIsoline iso, Vector2D dir) {
    ...
    /**
     * Rasterize isoline to buffer grid. see {@link DistanceFieldInterpolation#rasterizeline(pixelInfo[][], int, int, int, int, AttributedIsoline,Vector2D)}.
     */
    public void rasterize(pixelInfo[][] buf, AttributedIsoline line, PointRasterizer rasterizer, AttributedIsoline iso) {
     ...
     */
    public void rasterize(int[][] buf, AttributedIsoline line, PointRasterizer rasterizer, int val) {
        for (int coord_index = 1; coord_index < line.coordinates.size(); ++coord_index) {
    ...
    public double[][] getAllInterpolatingPoints() {
        ...
        for (int isoline_id = 0; isoline_id != isolines.length; ++isoline_id) {
            ...
            AttributedIsoline iso = isolines[isoline_id];
